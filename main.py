...
def print_sync_warning(title: str):
    print()
    print(f"  !! {title} !!")
    print("     La synchronisation Xbox Cloud n'est peut-être pas terminée.")
    print("     Les sauvegardes extraites pour ce jeu peuvent être corrompues !")
    print("     Appuyez sur Entrée pour ignorer et continuer.")
    input()
...
def find_user_containers(pkg_name: str) -> List[Tuple[int | str, Path]]:
    ...
    if has_backups:
        print("  !! Le dossier de sauvegarde contient des copies de secours !!")
        print("     Les copies créées par l'application Xbox seront ignorées.")
...
def get_save_paths(
    ...
    if handler_name == "1c1f":
        ...
        raise Exception('Application Xbox Game Pass non compatible : "%s"' % store_pkg_name)
...
def safe_zip_members(save_zip: zipfile.ZipFile) -> List[zipfile.ZipInfo]:
    ...
    for member in save_zip.infolist():
        ...
        if unix_file_type == 0o120000:
            raise ValueError(f'Lien symbolique refusé : "{member.filename}"')
...
def install_save_archive(archive_path: Path, destination: Path) -> Path | None:
    ...
    if archive_path.suffix.lower() != ".zip":
        raise ValueError("L'archive de sauvegarde sélectionnée doit être un fichier ZIP.")

    with zipfile.ZipFile(archive_path, "r") as save_zip:
        bad_member = save_zip.testzip()
        if bad_member is not None:
            raise ValueError(f'Fichier corrompu dans l\'archive : "{bad_member}"')
        members = safe_zip_members(save_zip)
        if not members:
            raise ValueError("L'archive sélectionnée ne contient aucune sauvegarde.")

        print()
        print(f'Archive: "{archive_path}"')
        print(f'Dossier de sauvegarde Steam : "{destination}"')
        print("Fichiers à copier :")
        for member in members:
            target = destination.joinpath(*PurePath(member.filename.replace("\\", "/")).parts)
            action = "remplacer" if target.exists() else "créer"
            print(f"- {member.filename} ({action})")

        print()
        answer = input("Continuer l'importation ? Tapez OUI pour confirmer : ").strip()
        if answer.upper() != "OUI":
            print("Importation annulée. Aucun fichier n'a été modifié.")
            return None
    ...
    print()
    print(f"Importation terminée : {len(members)} fichier(s) copié(s).")
    if backup_path is not None:
        print(f'Anciens fichiers Steam sauvegardés dans "{backup_path}"')
    return backup_path


def import_saves_to_steam():
    print("Importer une sauvegarde Xbox extraite dans un jeu Steam")
    print("========================================================")
    print("Fermez les versions Xbox et Steam du jeu avant de continuer.")
    print("Sinon, Steam Cloud pourrait restaurer une ancienne sauvegarde.")
    print()
    archive_path = clean_user_path(input("Chemin du ZIP Xbox extrait : "))
    destination = clean_user_path(input("Chemin du dossier de sauvegarde Steam : "))
    install_save_archive(archive_path, destination)


def choose_number(title: str, choices: List[Tuple[str, Any]]) -> Any:
    if not choices:
        raise ValueError(f"Aucun élément disponible : {title}")
    print(title)
...
def copy_palworld_world(
    ...
    if segmented_level.is_dir():
        ...
        timestamp = datetime.now().strftime("%Y-%m-%d_%H_%M_%S")
        retired = retired_root / f"ancien_Level_{world_dir.name}_{timestamp}"
        counter = 1
        while retired.exists():
            retired = retired_root / (
                f"ancien_Level_{world_dir.name}_{timestamp}_{counter}"
            )
            counter += 1
...
def convert_palworld_data_for_steam(data: bytes) -> bytes:
    ...
    if converted[8:11] != b"PlM" or verified_gvas != raw_gvas:
        raise ValueError("La vérification de la conversion Xbox vers Steam a échoué.")
    return converted
...
def group_palworld_worlds(
    ...
    for world_root in world_roots:
        ...
        world_label = "/".join(world_root.parts) or "sauvegarde principale"
...
def transfer_palworld_xbox_to_steam():
    print("Transfert automatique Palworld : Xbox Game Pass vers Steam")
    print("==========================================================")
    print("Avant de continuer :")
    print("- créez et sauvegardez un monde temporaire dans la version Steam ;")
    print("- fermez complètement Palworld, l'application Xbox et Steam.")
    print()

    games = read_game_list()
    package_name = "PocketpairInc.Palworld_ad4psfrxyesvt"
    if games is None or package_name not in games:
        raise ValueError("La configuration Palworld est absente de games.json.")

    xbox_users = find_user_containers(package_name)
    if not xbox_users:
        raise FileNotFoundError("Aucune sauvegarde Palworld Xbox Game Pass trouvée.")
    if len(xbox_users) > 1:
        xbox_user = choose_number(
            "Choisissez le profil Xbox :",
    ...
    try:
        ...
        if not xbox_worlds:
            print("Fichiers Xbox détectés :")
            for relative, _ in converted_paths:
                print(f"- {relative}")
            raise ValueError(
                "Aucun fichier Level.sav ou Level/01.sav n'a été trouvé. La synchronisation Xbox Cloud "
                "est peut-être incomplète : lancez Palworld via Xbox, chargez le monde, "
                "quittez proprement le jeu et réessayez après quelques minutes."
            )
        ...
        for world_id, files in xbox_worlds.items():
            ...
            xbox_choices.append(
                (f"Monde {world_id} — dernière sauvegarde : {modified}", (world_id, files))
            )
        if len(xbox_choices) == 1:
            xbox_world_id, source_files = xbox_choices[0][1]
            print(f"Monde Xbox détecté : {xbox_choices[0][0]}")
        else:
            xbox_world_id, source_files = choose_number(
                "Choisissez le monde Xbox à transférer :", xbox_choices
            )

        steam_worlds = find_steam_palworld_worlds()
        if not steam_worlds:
            raise FileNotFoundError(
                "Aucun monde Steam trouvé. Lancez Palworld sur Steam, créez un monde, "
                "sauvegardez-le, puis fermez le jeu."
            )
        steam_choices = []
        for account_dir, world_dir in steam_worlds:
            ...
            steam_choices.append(
                (
                    f"Compte {account_dir.name}, monde {world_dir.name} — {modified}",
                    (account_dir, world_dir),
                )
            )
        account_dir, world_dir = choose_number(
            "Choisissez le monde Steam temporaire à remplacer :", steam_choices
        )

        print()
        print(f"Profil Xbox : {xbox_user_name}")
        print(f"Monde Xbox source : {xbox_world_id}")
        print(f"Monde Steam cible : {world_dir}")
        print("Fichiers du monde qui seront remplacés :")
        for relative, _ in source_files:
            print(f"- {relative}")
        for relative, _ in root_files:
            print(f"- {relative.name}")
        print()
        answer = input("Tapez OUI pour lancer le transfert : ").strip().upper()
        if answer != "OUI":
            print("Transfert annulé. Aucun fichier n'a été modifié.")
            return
        ...
        print()
        print("Transfert Palworld terminé.")
        print(f'Sauvegarde de sécurité créée dans : "{backup_path}"')
        print("Vous pouvez maintenant lancer Palworld sur Steam.")
        print("Si Steam Cloud demande quelle sauvegarde conserver, choisissez les fichiers locaux.")
    finally:
        temp_dir.cleanup()


def extract_xbox_saves():
    print("Extracteur de sauvegardes Xbox Game Pass pour PC")
    print("=================================================")

    games = read_game_list()
    if games is None:
        print("Impossible de lire la liste des jeux. Vérifiez le fichier games.json.")
        print()
    ...
    if len(found_games) == 0:
        print("Aucun jeu compatible installé")
        print()
        print("Appuyez sur Entrée pour quitter")
        input()
        sys.exit(1)

    print("Jeux compatibles installés :")
    for package_name in found_games:
        ...
        try:
            ...
            if len(user_containers) == 0:
                print(
                    "  Aucune sauvegarde trouvée ; le jeu n'est peut-être plus installé"
                )
                print()
                continue
            ...
            for xbox_username_or_id, container_dir in user_containers:
                ...
                if len(save_paths) == 0:
                    continue
                print(f"  Sauvegardes de l'utilisateur {xbox_username_or_id} :")
                ...
                print()
                print('  Sauvegardes enregistrées dans "%s"' % zip_name)
                print()

        except Exception:
            print("  Échec de l'extraction des sauvegardes :")
            traceback.print_exc()
            print()
...
def test_palworld_conversion_engine():
    ...
    if games is None or not xbox_users:
        raise ValueError("Sauvegarde Xbox Palworld introuvable pour le test.")
    ...
    try:
        ...
        (world_dir / "Level.sav").write_bytes(b"ancien manifeste de test")
        (world_dir / "Level/01.sav").write_bytes(b"ancien segment de test")
        ...
        if level[8:11] != b"PlM" or (world_dir / "Level").exists():
            raise ValueError("Le test du format Steam a échoué.")
        from palsav.core import decompress_sav_to_gvas

        raw_level, _ = decompress_sav_to_gvas(level)
        if raw_level[:4] != b"GVAS":
            raise ValueError("Le Level.sav converti est invalide.")
        print("TEST PALWORLD RÉUSSI : CNK/PLZ Xbox converti en PLM Steam.")
    finally:
...
def main():
    ...
    print("Outil de transfert de sauvegardes Xbox Game Pass / Steam")
    print("=========================================================")
    print("1. Extraire les sauvegardes Xbox Game Pass dans un ZIP")
    print("2. Transférer automatiquement Palworld de Xbox Game Pass vers Steam")
    print("3. Importer manuellement un ZIP dans un dossier de sauvegarde Steam")
    print()
